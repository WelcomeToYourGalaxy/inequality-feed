#!/usr/bin/env python3
"""
harvest_inequality.py — the inequality wire: who owns what, who is priced out,
who writes the rules, and who carries the loss.

After the Economic Inequality Within It section of Suppression. It opens where
that section opens — with self-sufficiency, and the fact that the freedom not
to take part in a system requires the means to live outside it — and then
follows the section into what it calls the most powerful driver: the market.
Wages and executive pay, wealth and inheritance, mobility, purchasing power,
credit, region, discrimination, public services and tax; then unequal access to
lawmakers; then front running and the speed advantage, insider trading and the
gap between it and prosecution, capture of financial rulemaking, who tells you
what the market is, the esoteric terminology that hides the door in, pensions
as captive capital, derivatives and speculation, shareholder concentration and
who actually owns the market, the trading that happens away from the exchange,
the shift into private markets and who is allowed to invest there, what stops
being disclosed as a result, concentration among the too-big-to-fail, crashes
and who carries the loss, and the loopholes and buybacks.

A share price move or an earnings beat is not this subject. Who owns, who is
priced out, who writes the rules, and who carries the loss, is. Every subject
...
SOURCES_PATH = os.path.join(HERE, "sources_inequality.json")
OUT_PATH = os.path.join(HERE, "wire_inequality.json")
...
# --------------------------------------------------------------------------
USER_AGENT = ("Mozilla/5.0 (compatible; inequality-feed/1.0; "
...
# THE SUBJECTS
#
# Taken from the Economic Inequality Within It section: the facets of
# inequality it lists, and the market machinery it identifies as the driver.
#
# Each subject is (term, context) pairs. The term must appear AND one of its
# context words, so a market report or a stock tip cannot reach the wire.
# --------------------------------------------------------------------------
TOPICS = [
    ...
    ("wages", "Wages, executive pay and job security", [
        ("wage stagnation", []), ("real wages", ["fell", "fall", "rose", "stagnant", "decline", "squeeze"]),
        ("minimum wage", ["raise", "freeze", "ruling", "campaign", "below", "living"]),
        ("living wage", []), ("executive pay", []), ("ceo pay", []),
        ("pay ratio", []), ("bonus", ["executive", "banker", "cap", "record", "shareholder"]),
        ("gig economy", ["pay", "rights", "ruling", "classification", "precarious"]),
        ("zero-hours", []), ("precarious work", []), ("job security", ["fell", "decline", "erosion"]),
        ("layoffs", ["record", "profits", "buyback", "while", "despite"]),
        ("labour share", []), ("labor share", []),
    ]),
    ("wealth", "Wealth, inheritance and dynasty", [
        ("wealth inequality", []), ("wealth gap", []), ("wealth concentration", []),
        ("inheritance", ["tax", "wealth", "transfer", "dynasty", "concentration"]),
        ("intergenerational", ["wealth", "transfer", "mobility", "inequality"]),
        ("billionaire", ["wealth", "tax", "number", "fortune", "grew", "list"]),
        ("great wealth transfer", []), ("family office", []), ("dynasty trust", []),
        ("net worth", ["top", "share", "richest", "concentration"]),
        ("gini", []), ("top 1%", []), ("richest", ["own", "share", "percent", "wealth"]),
    ]),
    ("mobility", "Social mobility and opportunity", [
        ("social mobility", []), ("intergenerational mobility", []),
        ("opportunity gap", []), ("parental education", []), ("parental income", []),
        ("class ceiling", []), ("life chances", []),
        ("postcode", ["outcomes", "lottery", "determines"]),
        ("upward mobility", []), ("stuck", ["income bracket", "poverty", "generation"]),
    ]),
    ("consumption", "Purchasing power and basic necessities", [
        ("cost of living", []), ("purchasing power", []), ("real incomes", ["fell", "squeeze", "eroded"]),
        ("food insecurity", []), ("fuel poverty", []), ("energy poverty", []),
        ("food bank", ["demand", "record", "rise", "use"]),
        ("inflation", ["hits poorest", "unequal", "burden", "essentials", "food prices"]),
        ("shrinkflation", []), ("basic necessities", []), ("housing costs", ["share of income", "burden", "rise"]),
        ("rent burden", []), ("evictions", ["rise", "record", "wave"]),
    ]),
    ("credit", "Access to credit and capital", [
        ("access to credit", []), ("credit availability", []), ("loan availability", []),
        ("redlining", []), ("lending discrimination", []), ("mortgage denial", []),
    ...
    ("regional", "Regional and spatial inequality", [
        ("regional inequality", []), ("spatial inequality", []), ("left behind", ["region", "town", "area"]),
        ("infrastructure gap", []), ("rural", ["broadband", "investment", "neglect", "decline", "services"]),
        ("levelling up", []), ("regional divide", []), ("urban rural", ["gap", "divide", "inequality"]),
        ("investment gap", ["region", "north", "south", "province"]),
        ("development gap", []), ("north-south divide", []),
    ]),
    ("paygap", "Discrimination in the labour market", [
        ("gender pay gap", []), ("pay gap", ["gender", "ethnic", "racial", "disability", "report"]),
        ("wage discrimination", []), ("labour market discrimination", []),
        ("labor market discrimination", []), ("hiring discrimination", []),
        ("equal pay", ["ruling", "claim", "case", "law", "audit"]),
        ("occupational segregation", []), ("care work", ["unpaid", "valued", "burden", "economy"]),
    ...
    ("taxation", "Taxation and who pays", [
        ...
        ("effective tax rate", []), ("wealth tax", []), ("capital gains tax", []),
        ("corporate tax", ["rate", "cut", "avoidance", "minimum", "revenue"]),
        ("progressive taxation", []), ("regressive", ["tax", "vat", "levy"]),
        ("offshore", ["wealth", "assets", "accounts", "leak", "held"]),
        ("minimum tax", []), ("tax the rich", []), ("tax justice", []),
    ]),
    ("lawmakers", "Unequal access to lawmakers", [
        ("lobbying", ["finance", "bank", "spending", "disclosure", "influence", "wall street"]),
        ("campaign finance", []), ("political donation", []), ("dark money", []),
        ("revolving door", ["regulator", "treasury", "central bank", "finance", "sec"]),
        ("access to lawmakers", []), ("corporate influence", ["policy", "legislation", "regulator"]),
        ("political inequality", []), ("policy influence", ["wealthy", "donor", "affluent", "corporate"]),
    ]),
    # ----------------------------------------------------------------------
    # The market, kept only where it bears on who ends up with what. The
    # section's own point is that a tiny group concentrates the wealth everyone
    # else produces, that the field is made by experts for experts, and that
    # the law reserves the best of it for the already-rich. Market mechanics
    # that do not carry that — trading plumbing, enforcement, systemic risk —
    # are not this wire's subject.
    # ----------------------------------------------------------------------
    ("ownership", "Who owns the market", [
        ("blackrock", []), ("vanguard", ["stake", "holding", "shareholder", "voting", "assets", "owns"]),
        ("state street", ["stake", "holding", "shareholder", "voting"]),
        ("common ownership", []), ("blockholder", []),
        ("largest shareholder", []), ("concentration of ownership", []),
        ("share ownership", ["distribution", "concentration", "household", "who owns", "unequal"]),
        ("equity ownership", ["distribution", "concentration", "share", "unequal", "who"]),
        ("household", ["shares", "equity", "stock ownership", "own no", "do not own", "per cent of"]),
        ("listed equity", []), ("shareholder concentration", []),
        ("own", ["per cent of listed", "share of equity", "of the market", "of all shares"]),
        ("stock ownership", ["household", "share", "concentration", "unequal", "richest", "bottom"]),
        ("shareholder power", []), ("beneficial ownership", []),
        ("asset manager", ["holdings", "power", "influence", "owns", "control"]),
        ("capital income", ["share", "grew", "versus", "labour", "labor"]),
        ("labour share", ["fell", "decline", "versus capital"]),
        ("labor share", ["fell", "decline", "versus capital"]),
    ]),
    ("barriers", "The ladder pulled up", [
        ("financial literacy", ["barrier", "gap", "low", "unequal"]),
        ("retail investors", ["misled", "losses", "disadvantage", "outmatched", "warned"]),
        ("small investors", ["disadvantage", "losses", "squeezed", "priced out"]),
        ("mis-selling", []), ("misselling", []), ("hidden fees", []),
        ("management fees", ["erode", "high", "returns", "cost savers"]),
        ("complexity", ["deliberate", "barrier", "product", "opaque", "excludes"]),
        ("opaque", ["product", "pricing", "fee", "structure", "excludes"]),
        ("small print", []), ("plain language", ["finance", "disclosure", "terms"]),
        ("expensive", ["technology", "research", "advantage", "data", "access"]),
        ("advantage", ["institutional", "wealthy", "insiders", "over ordinary", "over retail"]),
        ("priced out", ["market", "investing", "assets", "housing"]),
        ("wealth defence", []), ("wealth defense", []),
        ("financial advice", ["gap", "unaffordable", "access", "cost"]),
    ]),
    ("accredited", "Private markets and who is allowed in", [
        ...
        ("private markets", ["access", "restricted", "wealthy", "institutional", "growth", "shift"]),
        ("private equity", ["access", "wealthy", "institutional", "returns to", "fees", "delisting"]),
        ("listed companies", ["fewer", "decline", "shrink", "falling", "number"]),
...
ANCHOR = [
    # Inequality language. A share price move, an earnings beat, an enforcement
    # action or a systemic-risk warning is not this subject; who owns, who is
    # priced out, who writes the rules, and who carries the loss, is.
    "wealth inequality", "income inequality", "wealth gap", "pay gap", "gini",
    "wage stagnation", "executive pay", "ceo pay", "living wage", "labour share",
    "labor share", "social mobility", "intergenerational", "cost of living",
    "food insecurity", "fuel poverty", "food bank", "rent burden", "household debt",
    "access to credit", "redlining", "financial exclusion", "unbanked",
    "predatory lending", "regional inequality", "spatial inequality",
    "gender pay gap", "labour market discrimination", "labor market discrimination",
    "austerity", "public services", "tax avoidance", "tax haven", "wealth tax",
    "offshore", "tax justice", "lobbying", "campaign finance", "dark money",
    "revolving door", "billionaire", "richest", "top 1%", "bottom 50%",
    "poverty", "destitution", "inequality",
    # the market, only where it bears on who ends up with what
    "blackrock", "vanguard", "state street", "common ownership", "blockholder",
    "concentration of ownership", "share ownership", "stock ownership",
    "beneficial ownership", "shareholder power",
    "accredited investor", "club deal", "private placement", "listed companies",
    "delisting", "take-private", "private markets", "private equity",
    "retail investors", "financial literacy", "mis-selling", "hidden fees",
    "priced out", "financial advice",
    "self-sufficiency", "food sovereignty", "commons",
]

BLOCK = [
    # the daily market report and the tip sheet
    "shares rose", "shares fell", "stock rose", "stock fell", "closed higher",
    "closed lower", "opening bell", "premarket", "pre-market movers", "after hours",
    "earnings beat", "earnings miss", "quarterly results", "guidance raised",
    "price target", "analyst upgrade", "analyst downgrade", "buy rating",
    "top stocks to buy", "best stocks", "stocks to watch", "hot stocks",
    "dividend yield of", "penny stock", "crypto price prediction", "should you buy",
    "here's why the stock", "motley fool", "zacks", "jim cramer",
    # promotion
    "sponsored content", "press release", "partner content", "coupon",
    "trading course", "signals group", "get rich", "passive income guide",
    # market machinery that is not this subject: trading plumbing, enforcement,
    # systemic risk and tax mechanics belong elsewhere unless a story ties them
    # to who ends up with what
    "front running", "high-frequency trading", "latency arbitrage", "dark pool",
    "payment for order flow", "best execution", "quote stuffing", "spoofing",
    "insider trading charge", "market abuse case", "securities fraud trial",
...
DECIDED = [
    ...
    "fined", "settled", "charged", "convicted", "banned", "ordered", "blocked",
    "nationalised", "nationalized", "bailed out", "delisted", "taken private",
]
INSTITUTIONAL = [
    "international monetary fund", "world bank", "united nations", "oecd",
    "bank for international settlements", "financial stability board", "central bank",
    "federal reserve", "european central bank", "bank of england", "finance ministry",
    "treasury", "securities and exchange commission", "financial conduct authority",
    ...
    "peer-reviewed", "published in", "working paper", "study finds", "dataset",
    "official data", "government figures", "national statistics", "world inequality",
    "household survey", "census",
]
MEASURED = [
    "per cent", "percent", "%", "billion", "million", "trillion", "quadrillion",
    "basis points", "share of gdp", "per capita", "median", "average", "ranked",
    "index", "figures show", "fell by", "rose by", "increase of", "decrease of",
    "estimated", "gini coefficient", "top 1%", "bottom 50%", "share of wealth",
    "market share", "notional", "assets under management",
]
PENDING = [
    "proposed", "draft law", "consultation", "under review", "expected to",
    "due to decide", "talks", "negotiation", "in negotiations", "deadline",
    "next month", "next year", "review scheduled", "pending approval", "pilot",
    "green paper", "white paper", "bill", "reading", "vote scheduled",
]
...
LOCAL_TERMS = {
    "paygap": [
        ("beloningsverhouding bestuurder werknemer", None), ("brecha salarial de", None),
        ("disparidade salarial de", None), ("divario retributivo di", None),
        ("gender pay gap", None), ("loonkloof rapport", None),
        ("luka płacowa raport", None), ("lönegap mellan könen", None),
        ("rapporto tra stipendio", None), ("ratio salarial del", None),
        ("rácio salarial do", None), ("vd-lön i förhållande", None),
        ("verhältnis vorstandsgehalt beschäftigte", None), ("wynagrodzenie prezesa a", None),
        ("écart de rémunération", None), ("écart salarial femmes-hommes", None),
        ("гендерный разрыв в", None), ("зарплата гендиректора и", None),
        ("فجوة الأجور بين", None), ("نسبة أجر الرئيس", None),
        ("役員報酬 従業員 比率", None), ("性别薪酬差距 报告", None),
        ("男女賃金格差 報告", None), ("高管薪酬 员工 倍数", None),
        ("성별 임금 격차", None), ("임원 보수 직원", None),
    ],
    "services": [
        ("cortes nos serviços", None), ("coupes dans les", None),
        ("crise da habitação", None), ("crise du logement", None),
        ("crisi abitativa", None), ("crisis de vivienda", None),
        ("desigualdad en el", None), ("desigualdade no acesso", None),
        ("disuguaglianza accesso alle", None), ("inégalités d'accès aux", None),
        ("kamu hizmetlerinde kesinti", None), ("ketimpangan akses kesehatan", None),
        ("konut krizi", None), ("krisis perumahan", None),
        ("kürzungen öffentliche dienste", None), ("pemangkasan layanan publik", None),
        ("recortes en servicios", None), ("sağlığa erişimde eşitsizlik", None),
        ("tagli ai servizi", None), ("ungleicher zugang gesundheit", None),
        ("wohnungskrise", None), ("ανισότητα πρόσβασης στην", None),
        ("περικοπές δημόσιων υπηρεσιών", None), ("жилищный кризис", None),
        ("неравный доступ к", None), ("сокращение социальных услуг", None),
        ("住宅費 高騰", None), ("住房 危机", None),
        ("公共サービス 削減", None), ("公共服务 削减", None),
        ("医疗可及性 不平等", None), ("医療アクセス 格差", None),
        ("공공서비스 축소", None), ("의료 접근성 격차", None),
        ("주거비 위기", None),
    ],
    "taxation": [
        ("belastingontwijking multinational", None), ("elisão fiscal multinacional", None),
        ("elusione fiscale multinazionali", None), ("elusión fiscal multinacional", None),
        ("filtración offshore", None), ("fuga offshore", None),
        ("fuite offshore", None), ("förmögenhetsskatt förslag", None),
        ("imposto sobre a", None), ("impuesto a la", None),
        ("impôt sur la", None), ("offshore-leak", None),
        ("pajak kekayaan usulan", None), ("patrimoniale proposta", None),
        ("penghindaran pajak perusahaan", None), ("podatek majątkowy propozycja", None),
        ("servet vergisi önerisi", None), ("skatteflykt multinationella", None),
        ("steuervermeidung konzern", None), ("unikanie opodatkowania korporacje", None),
        ("vergiden kaçınma çok", None), ("vermogensbelasting voorstel", None),
        ("vermögensteuer vorschlag", None), ("évasion fiscale multinationale", None),
        ("φοροαποφυγή πολυεθνικών", None), ("φόρος πλούτου πρόταση", None),
        ("налог на богатство", None), ("уклонение от налогов", None),
        ("التهرب الضريبي شركات", None), ("ضريبة الثروة اقتراح", None),
        ("オフショア 流出", None), ("多国籍企業 租税回避", None),
        ("富裕税 提案", None), ("离岸 泄露", None),
        ("财富税 提案", None), ("跨国公司 避税", None),
        ("다국적기업 조세회피", None), ("부유세 제안", None),
    ],
    "wages": [
        ("asgari ücret artışı", None), ("aumento del salario", None),
        ("aumento do salário", None), ("baisse des salaires", None),
        ("calo dei salari", None), ("caída de los", None),
        ("furto di salario", None), ("hausse du salaire", None),
        ("höjd minimilön", None), ("kenaikan upah minimum", None),
        ("lương thực tế", None), ("mindestlohn erhöhung", None),
        ("minimumloon verhoging", None), ("podwyżka płacy minimalnej", None),
        ("queda dos salários", None), ("reallöhne sinken", None),
        ("reel ücretlerde düşüş", None), ("reële lonen dalen", None),
        ("robo de salarios", None), ("roubo salarial", None),
        ("sjunkande reallöner", None), ("spadek płac realnych", None),
        ("subida del salario", None), ("tăng lương tối", None),
        ("upah riil turun", None), ("vol de salaire", None),
        ("αύξηση κατώτατου μισθού", None), ("πτώση πραγματικών μισθών", None),
        ("падение реальных зарплат", None), ("повышение минимальной зарплаты", None),
        ("تراجع الأجور الحقيقية", None), ("رفع الحد الأدنى", None),
        ("न्यूनतम मजदूरी वृद्धि", None), ("वास्तविक मजदूरी में", None),
        ("ขึ้นค่าแรงขั้นต่ำ", None), ("ค่าจ้างที่แท้จริงลดลง", None),
        ("实际工资 下降", None), ("実質賃金 低下", None),
        ("最低工资 上调", None), ("最低賃金 引き上げ", None),
        ("실질임금 하락", None), ("최저임금 인상", None),
    ],
    "wealth": [
        ("concentración de la", None), ("concentration des richesses", None),
        ("concentrazione della ricchezza", None), ("concentração da riqueza", None),
        ("el 1% más", None), ("fortuna de los", None),
        ("fortuna dos bilionários", None), ("fortune des milliardaires", None),
        ("förmögenhetskoncentration rapport", None), ("kekayaan miliarder", None),
        ("koncentracja majątku raport", None), ("konsentrasi kekayaan laporan", None),
        ("l'1% più ricco", None), ("les 1% les", None),
        ("majątki miliarderów", None), ("miljardärers förmögenhet", None),
        ("milliardärsvermögen", None), ("milyarderlerin serveti", None),
        ("o 1% mais", None), ("patrimoni dei miliardari", None),
        ("reichstes prozent", None), ("servet yoğunlaşması rapor", None),
        ("vermogen miljardairs", None), ("vermogensconcentratie rapport", None),
        ("vermögenskonzentration bericht", None), ("περιουσίες δισεκατομμυριούχων", None),
        ("συγκέντρωση πλούτου έκθεση", None), ("концентрация богатства доклад", None),
        ("состояние миллиардеров", None), ("تركز الثروة تقرير", None),
        ("ثروات المليارديرات", None), ("अरबपतियों की संपत्ति", None),
        ("धन का संकेंद्रण", None), ("亿万富豪 财富 增长", None),
        ("富の集中 報告", None), ("富裕層 資産 増加", None),
        ("财富集中 报告", None), ("부의 집중 보고서", None),
        ("억만장자 자산 증가", None),
    ],
}
...
FILL_TERMS = {
    "barriers": [
        ("acceso al crédito", None), ("accesso al credito", None),
        ("accès au crédit", None), ("acesso ao crédito", None),
        ("barreiras de licenciamento", None), ("barreras de licencias", None),
        ("barriere delle licenze", None), ("barrières des licences", None),
        ("berufszulassung als hürde", None), ("kreditzugang verweigert geringverdiener", None),
        ("低所得者 融資 拒否", None), ("低收入 信贷 被拒", None),
        ("职业许可 门槛", None), ("資格制度 参入障壁", None),
        ("저소득층 대출 거절", None), ("직업 면허 진입장벽", None),
    ],
    "selfsufficiency": [
        ("autossuficiência doméstica cultivo", None), ("autosufficienza domestica coltivazione", None),
        ("autosuffisance des ménages", None), ("autosuficiencia doméstica cultivo", None),
        ("banco de terras", None), ("banco de tierras", None),
        ("foncier solidaire", None), ("fondo fondiario comunitario", None),
        ("gemeinschaftsbodenfonds", None), ("netzwerk gegenseitiger hilfe", None),
        ("red de apoyo", None), ("rede de apoio", None),
        ("rete di mutuo", None), ("réseau d'entraide économie", None),
        ("selbstversorgung haushalte anbau", None), ("コミュニティ土地信託", None),
        ("互助 网络", None), ("家庭 自给自足 种植", None),
        ("相互扶助 ネットワーク", None), ("社区土地信托", None),
        ("自給自足 家庭 菜園", None), ("공동체 토지 신탁", None),
        ("상호부조 네트워크", None), ("자급자족 가정 텃밭", None),
    ],
}
...
def run(dry_run=False, fixtures=None):
    ...
    payload = {
        ...
        "languages": languages,
        "standings": [
            {"id": "official", "label": "Regulators, central banks & institutions"},
            {"id": "research", "label": "Research & inequality data"},
            {"id": "press", "label": "Press"},
            {"id": "rights", "label": "Campaigning & accountability groups"},
        ],
    ...
    if dry_run:
        print("\n--dry-run: wire_inequality.json not written")
        return payload
